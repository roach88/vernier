...
assert(smokeLoop, "installed smoke config did not register control-plane-smoke-test")
...
run("node", ["--input-type=module", "-e", "import('vernier').then((m) => { if (!m.sig || !m.defineLoop) throw new Error('missing public exports') })"], {
  cwd: consumerDir,
})
