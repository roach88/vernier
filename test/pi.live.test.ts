...
//
//   LOOPER_LIVE=1 LOOPER_LIVE_PI=1 npm test -- pi.live
...
const LIVE_PI = process.env.LOOPER_LIVE === "1" && process.env.LOOPER_LIVE_PI === "1"
...
function spec(): StepSpec {
  return {
    ...
    iteration: 1,
    inputs: {},
    prompt: "Reply with exactly this text: looper pi live proof",
    effects: noEffects(),
    runDir: mkdtempSync(join(tmpdir(), "looper-pi-live-run-")),
    timeoutMs: 180_000,
  }
}
...
describe("pi live proof", () => {
  it.skipIf(!LIVE_PI)(
    ...
    async () => {
      ...
      const result = await executor.run(spec(), { workdir: mkdtempSync(join(tmpdir(), "looper-pi-live-work-")) })
