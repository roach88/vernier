...
import { KEY_VERSION, type ContractEntry, type DecisionEntry, type EffectsEntry, type RunMetaEntry, type StepResultEntry, type StepStartedEntry } from "../src/ledger/ledger.js"
...
function meta(overrides: Partial<RunMetaEntry> = {}): RunMetaEntry {
  return {
    type: "meta",
    runId: "run-1",
    traceId: "trace-1",
...
function started(stepId: string, key = `${stepId}-1`, executorId = "codex"): StepStartedEntry {
  return { type: "step_started", key, stepId, iteration: 1, attempt: 1, executorId, at: at(1) }
}

function result(stepId: string, key = `${stepId}-1`, overrides: Partial<StepResultEntry> = {}): StepResultEntry {
  return {
    type: "step_result",
    key,
    stepId,
...
function contract(stepId: string, valid: boolean, key = `${stepId}-1`): ContractEntry {
  return {
    type: "contract",
    key,
    stepId,
...
function decision(stepId: string, kind: "continue" | "stop" | "escalate", classification: "success" | "failure", key = `${stepId}-1`): DecisionEntry {
  return {
    type: "decision",
    key,
    stepId,
...
describe("projectRunEvidence", () => {
  ...
  it("degrades legacy key versions instead of treating display-compatible journals as trust evidence", () => {
    const projection = projectRunEvidence({
      entries: [meta({ keyVersion: "loop-v1" }), started("route"), result("route"), effects("route", true), decision("route", "stop", "success")],
    })
  ...
  it("marks missing effects as degraded/unknown evidence", () => {
    const projection = projectRunEvidence({ entries: [meta(), started("write"), result("write"), decision("write", "stop", "success")] })
  ...
  it("keeps missing usage honest rather than fabricating availability or cost", () => {
    const resultWithoutUsage: StepResultEntry = JSON.parse(JSON.stringify(result("route", "route-1"), (_key, value) => (_key === "usage" ? undefined : value)))
    const projection = projectRunEvidence({
      entries: [meta(), started("route"), resultWithoutUsage, effects("route", true), decision("route", "stop", "success")],
    })
