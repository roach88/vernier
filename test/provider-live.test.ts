...
const LIVE_CURSOR_REQUESTED = process.env.LOOPER_LIVE === "1" && process.env.LOOPER_LIVE_CURSOR === "1"
const CURSOR_BIN = process.env.LOOPER_CURSOR_BIN ?? "cursor-agent"
...
function spec(): StepSpec {
  return {
    ...
    iteration: 1,
    inputs: {},
    prompt: "Reply with exactly this text: looper cursor live proof",
    effects: noEffects(),
    runDir: mkdtempSync(join(tmpdir(), "looper-cursor-live-run-")),
    timeoutMs: 180_000,
  }
}
...
describe("cursor-agent live proof", () => {
  it.skipIf(!CURSOR_AVAILABLE)(
    ...
    async () => {
      ...
      const result = await executor.run(spec(), { workdir: mkdtempSync(join(tmpdir(), "looper-cursor-live-work-")) })
