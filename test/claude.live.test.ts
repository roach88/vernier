...
//
//   LOOPER_LIVE=1 LOOPER_LIVE_CLAUDE=1 npm test -- claude.live
...
const LIVE_CLAUDE_REQUESTED = process.env.LOOPER_LIVE === "1" && process.env.LOOPER_LIVE_CLAUDE === "1"
...
function spec(): StepSpec {
  return {
    ...
    iteration: 1,
    inputs: {},
    prompt: "Reply with exactly this text: looper claude live proof",
    effects: noEffects(),
    runDir: mkdtempSync(join(tmpdir(), "looper-claude-live-run-")),
    timeoutMs: 180_000,
  }
}
...
describe("claude live proof", () => {
  it.skipIf(!CLAUDE_AVAILABLE)(
    ...
    async () => {
      ...
      const result = await executor.run(spec(), { workdir: mkdtempSync(join(tmpdir(), "looper-claude-live-work-")) })
