...
import { z } from "zod"
import { evaluateLoopQuality, noEffects, retryPolicy, sig, type Loop, type LoopQualityRuleId, type LoopQualityTarget } from "../src/index.js"
...
function failingRuleIds(target: LoopQualityTarget): LoopQualityRuleId[] {
  return evaluateLoopQuality(target).checks.filter((check) => check.status === "fail").map((check) => check.ruleId)
}

function expectFails(target: LoopQualityTarget, ruleId: LoopQualityRuleId): void {
...
describe("loop-quality evaluator", () => {
  it("passes every shipped template with stable rule IDs exercised", () => {
    const reports = loadedTemplates.map(({ name, registration }) => ({
      name,
      report: evaluateLoopQuality({ loop: registration.loop, live: registration.live }),
    }))
