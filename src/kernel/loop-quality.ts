...
const PROVIDER_IDS: Record<string, true> = { codex: true, claude: true, cursor: true, "cursor-agent": true, opencode: true, pi: true }
const LLM_EXECUTOR_IDS: Record<string, true> = { agent: true, judge: true, ...PROVIDER_IDS }

function hasKey(record: Record<string, true>, key: string): boolean {
...
function providerNeutralExecutorCheck(loop: Loop, step: Step): LoopQualityCheck {
  if (!hasKey(PROVIDER_IDS, step.executor)) {
...
function activeAgentLoopLiveCheck(target: LoopQualityTarget): LoopQualityCheck {
  const { loop } = target
  const usesAgent = loop.steps.some(isPromptBacked)
