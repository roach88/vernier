...
import { KEY_VERSION, replay, type EffectsEntry, type LedgerEntry, type StepResultEntry, type StepStartedEntry } from "./ledger.js"
...
export function projectRunEvidence(input: ProjectRunEvidenceInput): RunEvidenceProjection {
  ...
  const startedByKey = startedEntries(entries)
  const resultByKey = resultEntries(entries)
  const steps: StepRunEvidence[] = []

  for (const result of resultByKey.values()) {
    const started = startedByKey.get(result.key)
    if (!started) diagnostics.push({ severity: "corrupt", code: "ORPHAN_STEP_RESULT", detail: `step_result ${result.key} has no matching step_started entry` })
    const contract = view.contracts.get(result.key)
  ...
  for (const started of startedByKey.values()) {
    if (!resultByKey.has(started.key)) {
  ...
  for (const [key, contract] of view.contracts.entries()) {
    if (!resultByKey.has(key)) diagnostics.push({ severity: "corrupt", code: "ORPHAN_CONTRACT", detail: `contract for ${contract.stepId} has no matching step_result` })
  }
...
function startedEntries(entries: readonly LedgerEntry[]): Map<string, StepStartedEntry> {
  const out = new Map<string, StepStartedEntry>()
  for (const entry of entries) if (entry.type === "step_started") out.set(entry.key, entry)
  return out
}

function resultEntries(entries: readonly LedgerEntry[]): Map<string, StepResultEntry> {
  const out = new Map<string, StepResultEntry>()
  for (const entry of entries) if (entry.type === "step_result") out.set(entry.key, entry)
  return out
}
